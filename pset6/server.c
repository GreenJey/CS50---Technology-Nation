...
// Глобальні константи для масивів
...
int main(int argc, char* argv[])
{
    ...
    while (true)
    {
        ...
        if (connected()) // браузер підключився до сервера або ні 
        {
            // check for request
...
char* indexes(const char* path)
{
    ...
    char* path_php = malloc(length + 9);
    
    char* path_html = malloc(length + 10);
...
bool load(FILE* file, BYTE** content, size_t* length)
{
    ...
    *length = st.st_size;
    if (!(*length))
    {
        ...
        while((temp[*length - 1] = fgetc(file)) != EOF) 
            temp = realloc(temp, ++(*length));
        (*length)--;
        *content = temp;
    }
    else
        fread((*content = malloc(*length)), *length, 1, file);
    return true;
}
...
const char* lookup(const char* path)
{
    // Пошук першого вхідного символу після '.'
    char* ext;
    ext = strrchr(path, '.');
    
    // Порівняння двох рядків на співпадіння
    if(ext != NULL)
    {
        if(strcasecmp(".css", ext) == 0)
            return "text/css";
        if(strcasecmp(".html", ext) == 0)
            return "text/html";
        if(strcasecmp(".gif", ext) == 0)
            return "text/gif";
        if(strcasecmp(".ico", ext) == 0)
            return "text/ico";
        if(strcasecmp(".jpg", ext) == 0)
            return "text/jpg";
        if(strcasecmp(".js", ext) == 0)
            return "text/js";
        if(strcasecmp(".php", ext) == 0)
            return "text/php";
...
bool parse(const char* line, char* abs_path, char* query)
{
    ...
    // locate start of request_target
    const char* start = strstr(line, "/");
    // locate end of request_target
    const char* end = strstr(line, " HTTP/1.1");
    // creat array for request_target
    char request_target[end - start + 1];
    // copy from line chars into request_target
    strncpy(request_target, start, end - start);
    request_target[end - start] = '\0';
    // extract and write abs_path and query
    ...
    else
    {
        ...
        abs_path[strchr(request_target, '?') - request_target] = '\0';
        strcpy(query, strrchr(request_target, '?') + 1); 
    }
    return true;
}
